...
requirements_brief: docs/component-requirements/demo-widget.md
---

# DemoWidget Specification
...
| Input | Standing here |
|---|---|
| `docs/component-requirements/demo-widget.md` (stage #0, on `spec/demo-widget-requirements`) | **Normative.** Its `CR-001`–`CR-018` are what this component must do. Its repo-audit fences are binding facts about *this* repository. |
...
`DemoWidget` answers one question for a user part-way through a linear flow: **where am I,
...
DemoWidget is **inert**: nothing focusable, nothing clickable, no interactive state, and no
...
```
root                      data-slot="demo-widget"
                          flex column, gap-2, min-w-min, height derived (D6)
├── text row              data-slot="demo-widget-text"
...
**There is no `tokens/demo-widget.json` and none is proposed.** Nothing here needs a value
